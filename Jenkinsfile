pipeline {
  ...
  post {
    always {
     ...
     failure {  
             mail bcc: '', body: "<b>Portal access is failing, check on priority</b><br>Project: ${env.JOB_NAME} <br>Build Number: ${env.BUILD_NUMBER} <br> URL de build: ${env.BUILD_URL}", cc: '', charset: 'UTF-8', from: '', mimeType: 'text/html', replyTo: '', subject: "ERROR CI: Project name -> ${env.JOB_NAME}", to: "swathrao@cisco.com";  
         } 
}
}
